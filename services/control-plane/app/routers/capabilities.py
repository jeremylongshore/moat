...
class CapabilityCreateRequest(BaseModel):
    ...
    name: str = Field(..., min_length=1, max_length=128, description="Human-readable capability name")
    description: str = Field(..., min_length=1, max_length=2048, description="What this capability does")
    provider: str = Field(..., min_length=1, max_length=64, description="Provider identifier (e.g. 'openai', 'anthropic')")
    version: str = Field(..., pattern=r"^\d+\.\d+\.\d+$", description="Semver capability version")
    input_schema: dict[str, Any] = Field(default_factory=dict, description="JSON Schema for accepted input parameters")
    output_schema: dict[str, Any] = Field(default_factory=dict, description="JSON Schema for returned output")
...
async def list_capabilities(
    ...
    records = capability_store.list(provider=provider, status=status)
...
async def get_capability(capability_id: str) -> CapabilityResponse:
    """Fetch a single capability by its ID."""
    record = capability_store.get(capability_id)
    if record is None:
        raise HTTPException(
...
async def update_capability_status(
    ...
    record = capability_store.update_status(capability_id, new_status)
    if record is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Capability '{capability_id}' not found",
